...
หมายเหตุ: `bun run smoke:local` ต้องตรวจ local chat runtime เมื่อ backend health รายงานว่าใช้ local provider โดยยิง `POST /chat`, ตรวจว่ามี `chatId`, คำตอบยาวถึงขั้นต่ำ roleplay, model เป็น local runtime ที่คาดไว้, และ `totalTokens=0`
