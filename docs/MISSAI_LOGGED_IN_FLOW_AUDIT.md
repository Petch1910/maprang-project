...
- Public Gallery ใน Maprang เปิดเป็น local-ready opt-in flow แล้ว: publish/unpublish, sanitized list/detail, generation-output report/moderation path, reuse flow, private-by-default, no fake public data, และไม่ expose storage key
- backend generation foundation มี job/output/gallery routes, Prisma `GenerationJob` / `GenerationOutput`, owner-scoped output actions, signed/direct download boundary, และ blocked/no-debit local-safe behavior
...
- งานต่อที่ควรทำจากจุดนี้คือ `AICreatorPage.tsx` orchestration cleanup, remaining browser hardening, production moderation runbook, และ external live provider/storage/deployed-domain verification
