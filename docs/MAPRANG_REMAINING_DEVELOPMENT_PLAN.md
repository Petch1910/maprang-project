...
- Still open locally: clean code/refactor หน้าหนักที่ยังรวม orchestration เยอะ ได้แก่ `CharacterCreateForm` และ `WorkspacePage`. `AICreatorPage.tsx` ปิดงาน refactor หลักแล้วโดยแยก character options, local history persistence, upload reference validation, manual/template generation, download handling, library item actions, reference-to-Creator bridge, save-to-studio, clipboard copy actions, และ gallery history view state ออกเป็น hook แล้ว. `CharacterCreateForm` cleanup คืบหน้าแล้วโดยย้าย draft status/readiness/storage/payload helper ไป `creatorFormState.ts`, ย้าย remote draft persistence ไป `useCreatorDraftPersistence`, ย้าย upload/generation handlers ไป `useCreatorDraftGeneration`, และย้าย reset/cover-image actions ไป `useCreatorFormActions`; `WorkspacePage` cleanup คืบหน้าแล้วโดยย้าย runtime helpers, report flow, world-state save, stream-success cleanup, saved-chat history loading, และ saved-chat runtime state composition ไป hook/helper แล้ว. งานถัดไปคือแยก chat send orchestration หรือ route boot orchestration.
