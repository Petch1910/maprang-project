...
- next implementation target: production provider verification, video/advanced-video execution readiness, และ UI QA matrix ที่แสดง blocked states ให้เช็คได้จากหน้าเว็บโดยตรงเมื่อเพิ่ม template ใหม่
...
- ถ้า backend/moderation ไม่พร้อมในสภาพแวดล้อม production ให้ Public Gallery แสดง empty/error state ที่อธิบายชัดเจน แต่ห้ามใช้ข้อมูลสาธารณะปลอม
- local-safe implementation ที่มีตอนนี้: `/ai-creator` แสดง Public Gallery จาก public outputs ที่ผ่าน sanitized backend DTO เท่านั้น มี opt-in publish/unpublish, report/moderation path, reuse flow, และไม่มีข้อมูลสาธารณะปลอม
- next implementation target: production runbook/policy copy, richer public detail UX, browser hardening เท่าที่จำเป็น, และ external live provider/storage/deployed-domain verification
...
- `POST /generation/jobs/:id/cancel` - เสร็จแล้วสำหรับ owner-only cancel/no-extra-debit path
- `POST /generation/jobs/:id/retry` - เสร็จแล้วสำหรับ owner-only retry/no-duplicate-debit path
- `POST /generation/outputs/:id/use-as-character-image` - เสร็จแล้วสำหรับ owner-safe Creator Studio reference
- `POST /generation/outputs/:id/use-as-cover` - เสร็จแล้วสำหรับ owner-safe cover reference
...
- Public Gallery uses opt-in public visibility with sanitized DTOs, report/moderation guard, and no fake public data
