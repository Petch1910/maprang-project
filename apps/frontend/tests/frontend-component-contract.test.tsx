...
describe('frontend component contracts', () => {
  ...
  test('chat opening uses a structured intro card instead of duplicating the greeting bubble', () => {
    ...
    expect(source).toContain('buildOpeningChoices(character, runtimeState)')
    expect(chatSource).toContain('function openingSceneContent')
    expect(chatSource).toContain('export function buildOpeningChoices')
    expect(chatSource).toContain("tone === 'hostile'")
    expect(chatSource).toContain("tone === 'warm'")
    expect(chatSource).toContain('ทักแบบไม่ยอมถอย')
    expect(chatSource).toContain('เข้าใกล้อย่างอ่อนโยน')
