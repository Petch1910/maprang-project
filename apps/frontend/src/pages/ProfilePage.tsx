...
import {
  loadPersonaDraft,
  savePersonaDraft,
  savePersonaDraftToCloud,
  selectPersonaDraft,
  selectPersonaUpdatedAt,
...
const contentModes = [
  {
    label: 'ทั่วไป',
    detail: 'ซ่อนเนื้อหาเข้มข้น เหมาะกับผู้ใช้ทั่วไปและการทดสอบระบบพื้นฐาน',
    isAdult: false,
    maxRating: 'general',
  },
  {
    label: 'โรแมนซ์เบา',
    detail: 'เปิดความสัมพันธ์และอารมณ์โรแมนซ์แบบไม่จัดเต็ม',
    isAdult: false,
    maxRating: 'teen_romance',
  },
  {
    label: 'ผู้ใหญ่ 18+',
    detail: 'เปิดโหมดผู้ใหญ่สำหรับเนื้อเรื่องสมมุติ โดยยังอิงการตั้งค่าบัญชีและระบบรายงาน',
    isAdult: true,
...
function formatSavedAt(value: string) {
  if (!value) return 'ยังไม่มีการบันทึกในรอบนี้'
  const date = new Date(value)
  if (Number.isNaN(date.getTime())) return 'ยังไม่มีการบันทึกในรอบนี้'
...
export function ProfilePage() {
  ...
  const schedulePersonaCloudSave = (value: string, savedAtValue: string) => {
    ...
    personaSaveTimer.current = setTimeout(() => {
      void dispatch(savePersonaDraftToCloud(value))
        ...
        .catch(() => {
          setPersonaNote('บันทึกลงบัญชีไม่ได้ แต่ร่างในเครื่องยังอยู่')
  ...
  return (
    <main className="missai-shell grid gap-4 text-white lg:grid-cols-[minmax(0,1fr)_320px]">
      <section className="space-y-5">
        <section className="missai-card rounded-2xl p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <h1 className="font-display mt-2 text-3xl font-black text-white">ตัวตนและการตั้งค่าบัญชี</h1>
              <p className="mt-2 max-w-2xl text-sm font-bold leading-6 text-white/55">
                ข้อมูลนี้จะช่วยให้ AI เข้าใจบริบทผู้เล่นมากขึ้น และถูกบันทึกเป็นร่างในเครื่องก่อนซิงก์กับบัญชีเมื่อระบบหลังบ้านพร้อม
              </p>
            </div>
            <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1.5 text-xs font-black text-emerald-300">
              บันทึกอัตโนมัติ
            </span>
          </div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            ...
            <button className="missai-button-secondary" data-testid="profile-persona-clear" onClick={() => updatePersona('')} type="button">
              ล้างข้อมูล
            </button>
            <div className="missai-button-secondary flex text-white/45" data-testid="profile-persona-count">
          ...
          <div className="mt-4 rounded-xl border border-sky-500/20 bg-sky-500/10 p-4 text-sm font-bold leading-6 text-sky-300">
            เขียนให้กระชับจะได้ผลดีที่สุด เน้นตัวตน สไตล์การเล่น และขอบเขตสำคัญ มากกว่าประวัติยาวเกินจำเป็น
          </div>
          {personaNote && (
            <p className="missai-empty m-0 mt-3 px-3 py-2 text-xs text-white/58" data-testid="profile-persona-note">
              {personaNote}
            </p>
          )}
        </section>
          ...
          <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="m-0 mt-1 text-sm font-bold leading-6 text-white/55">
                ควบคุมสิ่งที่หน้าแนะนำและระบบแชทอนุญาตให้เห็น โดยระบบหลังบ้านจะจำกัดซ้ำตามบัญชี
              </p>
            </div>
            <span className="missai-badge text-white/65">{contentSettings.maxRating}</span>
          </div>
          <div className="mt-4 grid gap-2 md:grid-cols-3">
            {contentModes.map((mode) => {
              const isActive = contentSettings.isAdult === mode.isAdult && contentSettings.maxRating === mode.maxRating
              const contentModeTitle = isContentSaving ? 'กำลังบันทึกโหมดคอนเทนต์' : `เลือกโหมด ${mode.label}`
              return (
                <button
        ...
        <section className="missai-card rounded-2xl p-5">
          <h2 className="m-0 flex items-center gap-2 text-xl font-black text-white">
            ...
            โหมดนักพัฒนาและ API ส่วนตัว
          </h2>
          <p className="mt-1 text-sm font-bold leading-6 text-white/55">
            ใช้สำหรับทดสอบระบบสนทนาและการสร้างภาพด้วยคีย์ของผู้ใช้เอง คีย์ชั่วคราวเก็บใน session ของแท็บนี้ ส่วนตู้เซฟบัญชีเก็บผ่าน backend
          </p>
          ...
          <label className="mt-4 flex cursor-pointer items-center gap-3">
            <input
              ...
              onChange={(event) => {
                ...
                saveSettings(event.target.checked, customApiKey, customApiProvider)
              }}
              type="checkbox"
            />
            <span className="text-sm font-bold text-white/80">เปิดโหมดทดสอบระบบด้วย API ส่วนตัว</span>
          </label>
            ...
            <div className="mt-4 space-y-4 border-t border-white/5 pt-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="block">
                  <span className="text-xs font-black text-white/68">ผู้ให้บริการ</span>
                  <select
                    className="missai-input mt-1 block min-h-11 rounded-xl p-2.5 text-sm"
                    onChange={(event) => {
                      setCustomApiProvider(event.target.value)
                      saveSettings(bypassEnabled, customApiKey, event.target.value)
                    }}
                ...
                <label className="block">
                  ...
                  <input
                    ...
                    placeholder="วางคีย์สำหรับทดสอบในแท็บนี้"
                    type="password"
                    value={customApiKey}
                  />
                </label>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <button
                  ...
                  title={customApiKey.trim() ? 'ทดสอบการเชื่อมต่อ' : 'กรอก API key เพื่อทดสอบการเชื่อมต่อ'}
                  type="button"
                >
                  {isTesting ? 'กำลังทดสอบ...' : 'ทดสอบการเชื่อมต่อ'}
                </button>
                {testResult && (
                  <span className={`text-xs font-bold ${testResult.ok ? 'text-emerald-300' : 'text-rose-300'}`}>
                    {testResult.message}
                  </span>
                )}
              </div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="m-0 mt-1 text-xs font-bold leading-5 text-white/50">
                      เก็บแบบเข้ารหัสฝั่ง backend และใช้เฉพาะตอนเปิดโหมดทดสอบระบบ
                    </p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      className="missai-button-secondary disabled:opacity-50"
