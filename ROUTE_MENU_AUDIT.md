...
| Admin Health | `/admin/health` | health refresh, production blocker summary, deploy checklist, route/menu audit | เห็น DB, AI, Supabase, storage, CORS, production blocker count, route audit และคำสั่ง final gate ในหน้าเดียว | ไม่มี disabled ถาวร รีเฟรช disabled ได้เฉพาะตอนเรียกข้อมูล | backend ล่มจะแสดงสิ่งที่ต้องแก้ |
