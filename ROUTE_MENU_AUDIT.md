...
| Admin Health | `/admin/health` | health refresh, production blocker summary, deploy checklist, route/menu audit, ลิงก์ตรวจพรอมป์, ลิงก์ทดสอบคุณภาพ | เห็น DB, AI, Supabase, storage, CORS, production blocker count, route audit, ขั้นต่อไปของแต่ละ blocker และคำสั่ง final gate ในหน้าเดียว พร้อมไป Prompt Inspector / Automated Evals ได้ | ไม่มี disabled ถาวร รีเฟรช disabled ได้เฉพาะตอนเรียกข้อมูล | backend ล่มจะแสดงสิ่งที่ต้องแก้ |
...
| Automated Evals | `/admin/evals` | admin key, รัน eval, scenario accordion, check result, failure summary | เรียก admin API เพื่อรัน deterministic golden roleplay evals ตรวจ prompt-control, lore, relationship/scene continuity และ token budget โดยไม่ยิงโมเดลจริง | guard ด้วย `ADMIN_API_KEY`; ปุ่มรัน disabled เมื่อไม่มี key หรือกำลังรัน | แสดงสถานะยังไม่ได้รัน eval และอธิบายว่าจะเห็น scenario/check/failure หลังรัน |
