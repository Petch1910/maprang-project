...
const promiseRejectContainerAliasPattern = new RegExp(String.raw`(?::\s*${promiseRejectAliasValue}|=\s*${readonlyContainerPrefix}(?:\(\s*)?\[\s*${promiseRejectAliasValue}|=\s*${readonlyContainerPrefix}(?:\(\s*)?\[[^\]\n;]*?,\s*${promiseRejectAliasValue}|new\s+Map(?:\s*<[^>]+>)?\s*\([\s\S]{0,240}?\[\s*["'][^"'\n]+["']\s*,\s*${promiseRejectAliasValue}|${objectFromEntriesContainerPrefix}[\s\S]{0,240}?\[\s*["'][^"'\n]+["']\s*,\s*${promiseRejectAliasValue}|${objectAssignContainerPrefix}[\s\S]{0,240}?:\s*${promiseRejectAliasValue}|${objectDefinePropertyContainerPrefix}[\s\S]{0,260}?\bvalue\s*:\s*${promiseRejectAliasValue}|${objectCreateContainerPrefix}[\s\S]{0,260}?\bvalue\s*:\s*${promiseRejectAliasValue}|(?:new\s+Set(?:\s*<[^>]+>)?|Array\s*\.\s*(?:from|of))\s*\([\s\S]{0,160}?(?:\[\s*)?${promiseRejectAliasValue})`, 'g')
