...
function validateLocalChatSmokeStream(
  ...
  if (reply.length < minReplyChars) throw new Error(`local roleplay stream คืนคำตอบสั้นเกินไป: ${reply}`)
...
if (live) {
  ...
  if (hasLocalChatRuntime(healthStatus)) {
    ...
    await check('POST /chat local roleplay QA', async () => {
      const payload = await readJson<{
        reply?: string
        chatId?: string | null
        usage?: {
    ...
    await check('POST /chat/stream local roleplay QA', async () => {
