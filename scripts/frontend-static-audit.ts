...
const collectionConstructorTypeArguments = String.raw`(?:\s*<[^;\n()]+>)?`
const collectionMapConstructorBase = String.raw`(?:(?:(?:window|globalThis)\s*(?:\?\.|\.)\s*)?(?:Weak)?Map|(?:window|globalThis)\s*(?:\?\.)?\s*\[\s*["'](?:Weak)?Map["']\s*\])`
const collectionSetConstructorBase = String.raw`(?:(?:(?:window|globalThis)\s*(?:\?\.|\.)\s*)?(?:Weak)?Set|(?:window|globalThis)\s*(?:\?\.)?\s*\[\s*["'](?:Weak)?Set["']\s*\])`
const collectionMapConstructorAccessor = String.raw`(?:${collectionMapConstructorBase}|\(\s*${collectionMapConstructorBase}\s*\))`
const collectionSetConstructorAccessor = String.raw`(?:${collectionSetConstructorBase}|\(\s*${collectionSetConstructorBase}\s*\))`
const collectionMutationContainerPrefix = String.raw`(?:\(\s*\b[A-Za-z_$][\w$]*\s*\)|\b[A-Za-z_$][\w$]*|\(\s*new\s+${collectionMapConstructorAccessor}${collectionConstructorTypeArguments}\s*\([^)]*\)\s*\)|\(\s*new\s+${collectionSetConstructorAccessor}${collectionConstructorTypeArguments}\s*\([^)]*\)\s*\)|new\s+${collectionMapConstructorAccessor}${collectionConstructorTypeArguments}\s*\([^)]*\)|new\s+${collectionSetConstructorAccessor}${collectionConstructorTypeArguments}\s*\([^)]*\))\s*(?:(?:\?\.|\.)\s*(?:set|add)|(?:\?\.)?\s*\[\s*["'](?:set|add)["']\s*\])\s*(?:\?\.)?\s*\(`
...
export const suspiciousPatterns: Array<{ pattern: RegExp; message: string; allowedFiles?: Set<string> }> = [
  ...
  { pattern: /\blocal\s+mock\b/gi, message: 'พบข้อความ local mock ใน frontend source; ใช้ "โหมดในเครื่อง" แทนสำหรับข้อความผู้ใช้' },
