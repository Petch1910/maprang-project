...
const collectionPrototypeMutationContainerPrefix = String.raw`(?:(?:(?:window|globalThis)\s*\.\s*)?(?:Weak)?Map\s*\.\s*prototype\s*(?:(?:\?\.|\.)\s*set|(?:\?\.)?\s*\[\s*["']set["']\s*\])|(?:(?:window|globalThis)\s*\.\s*)?(?:Weak)?Set\s*\.\s*prototype\s*(?:(?:\?\.|\.)\s*add|(?:\?\.)?\s*\[\s*["']add["']\s*\]))\s*(?:(?:\?\.|\.)\s*(?:call|apply|bind)|(?:\?\.)?\s*\[\s*["'](?:call|apply|bind)["']\s*\])\s*\(`
