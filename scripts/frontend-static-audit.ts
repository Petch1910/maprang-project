...
const collectionMutationContainerPrefix = String.raw`(?:\(\s*\b[A-Za-z_$][\w$]*\s*\)|\b[A-Za-z_$][\w$]*|\(\s*new\s+(?:Weak)?Map${collectionConstructorTypeArguments}\s*\([^)]*\)\s*\)|\(\s*new\s+(?:Weak)?Set${collectionConstructorTypeArguments}\s*\([^)]*\)\s*\)|new\s+(?:Weak)?Map${collectionConstructorTypeArguments}\s*\([^)]*\)|new\s+(?:Weak)?Set${collectionConstructorTypeArguments}\s*\([^)]*\))\s*(?:(?:\?\.|\.)\s*(?:set|add)|(?:\?\.)?\s*\[\s*["'](?:set|add)["']\s*\])\s*(?:\?\.)?\s*\(`
