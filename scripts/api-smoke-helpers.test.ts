...
describe('api smoke helpers', () => {
  ...
  test('keeps API smoke diagnostics Thai-first', async () => {
    ...
    expect(apiSmoke).toContain('POST /chat/stream local QA')
    expect(apiSmoke).toContain('local chat QA ต้องไม่คิดโทเคน')
    ...
    expect(apiSmoke).toContain('/messages?limit=5')
    expect(apiSmoke).toContain('messageWindow')
    expect(apiSmoke).toContain('mayHaveMoreBefore')
    expect(apiSmoke).toContain('messages เกิน window limit ที่ขอ')
