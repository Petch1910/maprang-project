...
describe('predeploy check wiring', () => {
  test('keeps predeploy diagnostics Thai-first', async () => {
    ...
    expect(predeploy).toContain('docs command audit ครอบ 345 refs')
    ...
    expect(predeploy).toContain(String.raw`[\\\\s\\\\S]*?,\\\\s*error\\\\b[\\\\s\\\\S]*?\\\\)`)
    expect(predeploy).toContain(String.raw`\\\\s*error\\\\b\\\\s*(?:,|\\\\))`)
    ...
    expect(predeploy).toContain("${consoleErrorWarnCallPrefix}[\\\\\\\\s\\\\\\\\S]*?,\\\\\\\\s*(?:\\\\\\\\(\\\\\\\\s*)?${rawErrorExpressionPatternFor('error')}")
    ...
    expect(predeploy).toContain("${consoleErrorWarnCallPrefix}\\\\\\\\s*(?:\\\\\\\\(\\\\\\\\s*)?${rawErrorExpressionPatternFor('error')}")
