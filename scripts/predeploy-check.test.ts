...
describe('predeploy check wiring', () => {
  test('keeps predeploy diagnostics Thai-first', async () => {
    ...
    expect(predeploy).toContain("console\\\\\\\\.(?:error|warn)\\\\\\\\s*\\\\\\\\([\\\\\\\\s\\\\\\\\S]*?,\\\\\\\\s*(?:\\\\\\\\(\\\\\\\\s*)?${rawErrorExpressionPatternFor('error')}")
    ...
    expect(predeploy).toContain("console\\\\\\\\.(?:error|warn)\\\\\\\\s*\\\\\\\\(\\\\\\\\s*(?:\\\\\\\\(\\\\\\\\s*)?${rawErrorExpressionPatternFor('error')}")
