...
`qa:live` รัน local QA gate แล้วตามด้วย `api:smoke:live` หนึ่งรอบ. รอบ live นี้ตรวจทั้ง chat และ image generation แล้ว จึงไม่ควร chain `smoke:chat` หรือ `smoke:image:live` ต่อ ยกเว้นกำลัง retry เฉพาะเส้นทางผู้ให้บริการที่ fail.
