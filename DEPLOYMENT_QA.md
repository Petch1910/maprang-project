...
ใน local runtime, `api:smoke` จะเพิ่ม `POST /chat local roleplay QA` และ `POST /chat/stream local roleplay QA` เพื่อยืนยัน `local/mock-roleplay`, `totalTokens=0`, ไม่มี provider failure, มีคำตอบ roleplay ที่ยาวพอ, และมี streamed delta จริง ก่อนยังคงข้ามเฉพาะ live provider routes ที่ต้องรันกับ staging/production.
