...
`qa:local` ใช้ `qa:repo` เป็นฐานก่อน แล้วค่อยเพิ่ม runtime smoke ที่ต้องมี backend/Postgres จริง ได้แก่ `smoke:doctor`, `smoke:local`, และ `api:smoke`. ตอนนี้ `smoke:local` ตรวจ local chat runtime ด้วยเมื่อ `/health` รายงานว่า active runtime เป็น local: ต้องยิง `POST /chat` และ `POST /chat/stream` ได้, ได้ `chatId`, คำตอบยาวถึง `MODEL_MIN_ROLEPLAY_REPLY_CHARS` หรือขั้นต่ำ 420 ตัวอักษร, stream ต้องมี delta/done event, คืน model local ที่คาดไว้, และไม่คิดโทเคน provider.
