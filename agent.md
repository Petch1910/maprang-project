...
- Current addendum 2026-05-31: raw Promise rejection guards now also reject retrieved `Promise.reject` calls via `Reflect.get(Promise, "reject")` and `Object.getOwnPropertyDescriptor(Promise, "reject")?.value`, including call/apply/bind and Reflect.apply forwarding.
- Current addendum 2026-05-31: latest full `bun run qa:repo` passed after Promise executor raw reject guard for arrow/function/optional-call/call-apply-bind/Reflect.apply executors, Promise reject alias guard, reflected `Reflect.apply(Promise.reject, ..., [error])` guard, retrieved `Reflect.get(Promise, "reject")` / descriptor value guard, Reflect.apply retrieved-target raw log guard, frontend raw UI Promise rejection guard, and backend raw route Promise rejection guard; docs command audit now covers 349 refs, backend remains 178 tests / 611 expects, and frontend static/route/build/bundle gates are green.
