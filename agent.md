...
- Backend tests ล่าสุดผ่าน 128 tests
- API smoke ล่าสุดผ่าน 26 pass, 1 skip สำหรับ live chat local mode
...
- Backend URL ไม่ใช่ localhost
- Frontend `VITE_API_BASE_URL` ชี้ backend จริง
- `CORS_ORIGINS` เป็น frontend domain จริง ไม่รวม localhost ใน production
