...
- Current addendum 2026-05-31: frontend static audit and backend security audit now also reject console call/apply raw error logs, including provider, route, and frontend catch-variable variants, so method forwarding cannot bypass safe-summary logging.
- Current addendum 2026-05-31: full deterministic `bun run qa:repo` passed after the console call/apply raw error log guard, with backend tests 178 pass / 611 expects, memory audit 36 Markdown files, docs command audit 346 refs, frontend static/route audit, frontend build/bundle budget, API audit, route/menu audit, and docs/memory/knowledge gates all green.
