...
- Deploy readiness logic is shared by `smoke:doctor` and `deploy:status`, so current staging/production blockers and next steps can be printed without duplicating blocker rules.
