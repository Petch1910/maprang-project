...
- API route audit now scans `apps/backend/index.ts` and covers `GET /`, while API smoke and browser e2e smoke verify the backend root identity response before `/health`, so the backend root endpoint cannot drift outside the coverage map; the route audit regression test locks this root coverage.
