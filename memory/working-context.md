...
- 2026-05-21: Frontend AuthPanel sign-in classifier ไม่ lower-case raw `error.message` ตรงแล้ว โดยเพิ่ม `authClassifierText`/`authSecretPatterns` เพื่อ sanitize secret-shaped value ก่อน match Supabase auth hints; frontend static audit และ predeploy guard บล็อก `error.message.toLowerCase()`/`String(error).toLowerCase()` regression แล้ว.
