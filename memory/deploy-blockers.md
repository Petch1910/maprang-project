...
- Repo-owned static/unit/build gate ล่าสุดผ่าน `bun run qa:repo` หลังเพิ่ม frontend/backend response JSON guards, response text guards, frontend central API fetch guard, deploy status/smoke doctor/shared smoke/readiness runner/API/image/live-chat/local smoke diagnostic redaction, และ Supabase Storage setup/check diagnostic redaction; blocker ที่เหลือยังเป็น environment/staging/live-provider จริง.
