...
- `smoke:chat`, `smoke:image:live`, and combined `api:smoke:live` now return JSON `handoffEvidence` objects with release-field labels such as `Chat smoke normal walletTransactionId` and `Image smoke elapsedMs`, so provider verification runs are copy-ready for `RELEASE_HANDOFF.md`.
