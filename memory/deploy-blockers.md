...
- `api:smoke:live` now prints the live image handoff evidence rows (`Image smoke provider`, source, URL kind, elapsed milliseconds), includes proven live provider values in the final JSON summary `handoffEvidence`, and uses the same fallback/placeholder/missing URL/SVG validation as `smoke:image:live`, so the remaining image blocker is a real provider/staging run, not missing repo-owned evidence wiring.
...
- `smoke:chat`, `smoke:image:live`, and combined `api:smoke:live` now return JSON `handoffEvidence` objects with release-field labels such as `Chat smoke normal walletTransactionId`, `Chat smoke stream walletTransactionId`, and `Image smoke elapsedMs`, so provider verification runs are copy-ready for `RELEASE_HANDOFF.md`.
