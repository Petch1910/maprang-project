...
- Backend route throw/return guards now cover raw promise rejections such as `Promise.reject(error)`, `Promise.reject(cause)`, `Promise.reject(error as Error)`, plus optional-chained and bracket-notation Promise reject variants in route files; this is repo-owned hardening, not an external deploy blocker.
...
- Frontend static audit now covers raw UI promise rejections such as `Promise.reject(error)`, `Promise.reject(problem)`, plus optional-chained and bracket-notation Promise reject variants in components/pages; this is repo-owned hardening, not an external deploy blocker.
