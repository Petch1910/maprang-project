...
- Repo-owned static/unit/build gate ล่าสุดผ่าน `bun run qa:repo` วันที่ 2026-05-31 หลัง decision `0023` dangerous link protocol contract, frontend raw UI error throw guard, cross-window messaging guard, frontend no-op submit guard, native dialog guard, event listener cleanup guard, AuthError response helper guard, frontend aria-disabled reason guard, placeholder-link guard, no-op handler guard, no-op submit guard, native dialog guard, event listener cleanup guard, dangerous link protocol guard, raw classifier/UI error spacing guards, iframe `srcDoc` guard, backend raw error log guards, backend raw route return guard, และ spaced assignment hardening; memory audit ครอบ 33 Markdown files, docs command audit ครอบ 341 refs, test coverage audit ครอบ 60 files / 33 root test scripts, eval 3 scenarios, import-cycle audit 123 files / 293 edges, backend tests 178 pass / 611 expects, API audit ครอบ 48 backend routes + 34 frontend helper calls, route/menu audit 14 surfaces, frontend static audit allowlist guard, aria-disabled reason guard, placeholder-link guard, no-op handler guard, no-op submit guard, native dialog guard, event listener cleanup guard, dangerous link protocol guard, raw UI error throw guard, cross-window messaging guard, backend raw route return guard, frontend build และ bundle budget ผ่าน; blocker ที่เหลือยังเป็น environment/staging/live-provider จริง.
- Full deterministic `bun run qa:repo` ล่าสุดผ่านหลัง frontend raw UI error throw guard, cross-window messaging guard, frontend no-op submit guard, native dialog guard, event listener cleanup guard, และ backend raw route return guard แล้ว: memory audit 33 Markdown files, docs command audit 341 refs, test coverage audit 60 files / 33 root test scripts, eval 3 scenarios, import-cycle audit 123 files / 293 edges, API audit 48 backend routes + 34 frontend helper calls, route/menu audit 14 surfaces, backend tests 178 pass / 611 expects, frontend static/route audit, frontend build และ bundle budget ผ่าน; blocker ที่เหลือยังเป็น environment/staging/live-provider จริง.
