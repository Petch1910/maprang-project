...
- Frontend static audit now covers raw UI promise rejections such as `Promise.reject(error)`, `Promise.reject(problem)`, optional-chained/bracket-notation Promise reject variants, and aliasing Promise reject before invocation in components/pages; this is repo-owned hardening, not an external deploy blocker.
