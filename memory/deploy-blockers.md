...
- Backend route throw/return guards now cover raw promise rejections such as `Promise.reject(error)`, `Promise.reject(cause)`, and `Promise.reject(error as Error)` in route files; this is repo-owned hardening, not an external deploy blocker.
...
- Frontend static audit now covers raw UI promise rejections such as `Promise.reject(error)` and `Promise.reject(problem)` in components/pages; this is repo-owned hardening, not an external deploy blocker.
