...
- Backend route throw/return guards now cover raw promise rejections such as `Promise.reject(error)`, `Promise.reject(cause)`, `Promise.reject(error as Error)`, optional-chained/bracket-notation Promise reject variants, aliasing Promise reject before invocation, and Promise executor raw reject callbacks in both arrow/function-style route files including `reject?.(error)` optional calls plus `.call`/`.apply`/`.bind`, `Reflect.apply(reject, ...)`, and `globalThis.Reflect.apply(reject, ...)` forwarding; this is repo-owned hardening, not an external deploy blocker.
...
- Frontend static audit now covers raw UI promise rejections such as `Promise.reject(error)`, `Promise.reject(problem)`, optional-chained/bracket-notation Promise reject variants, aliasing Promise reject before invocation, and Promise executor raw reject callbacks in both arrow/function-style components/pages including `reject?.(error)` optional calls plus `.call`/`.apply`/`.bind`, `Reflect.apply(reject, ...)`, and `globalThis/window.Reflect.apply(reject, ...)` forwarding; this is repo-owned hardening, not an external deploy blocker.
