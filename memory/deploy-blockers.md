...
สถานะ repo-owned ล่าสุด: 2026-06-01 frontend/backend raw Promise rejection guards now reject retrieved Promise namespace access through `Reflect.get(..., 'Promise')` and `Object.getOwnPropertyDescriptor(..., 'Promise')?.value`, including direct `.reject(...)`, `Reflect.apply`, Promise object aliases, and retrieved `reject` aliases. Full repo QA passed with frontend static audit 39 tests / 145 expects, backend security audit 37 tests / 168 expects, predeploy lock 483 expects, backend tests 178 pass / 611 expects, docs command audit 353 refs, frontend build, and bundle budget green; external deploy blockers below are unchanged.
