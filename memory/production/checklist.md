...
สถานะ repo-owned ล่าสุด: 2026-06-01 frontend/backend raw Promise rejection guards now reject parenthesized/bracket Promise namespace access such as `(window).Promise.reject(error)`, `(globalThis).Promise.reject(error)`, and bracket-notation `globalThis` Promise/reject calls, including `.call`, `Reflect.apply`, and Promise object aliases. Full repo QA passed with frontend static audit 39 tests / 141 expects, backend security audit 37 tests / 166 expects, predeploy lock 480 expects, backend tests 178 pass / 611 expects, docs command audit 353 refs, frontend build, and bundle budget green; production checklist blockers remain external staging/live-provider work.
