...
- `bun run qa:repo` ล่าสุดผ่านหลัง frontend raw classifier/UI error spacing guards, iframe `srcDoc` guard, และ backend first-argument raw error log guard แล้ว พร้อม memory audit 33 Markdown files, backend tests 177 pass / 609 expects, frontend static/route audit, frontend build/bundle budget, API audit, route/menu audit, docs/memory/knowledge gates; DB persistence suites ยัง skip เฉพาะเพราะ local Postgres ไม่ได้รัน.
