...
- ยืนยัน focused frontend UI guard ล่าสุดด้วย `bun run frontend:static:audit:test` และ `bun run predeploy:check:test`; รอบ 2026-05-31 ปิด `Promise.reject(error)` / `Promise.reject(problem)` รวมถึง optional-chained, bracket-notation, alias Promise reject, และ Promise executor raw reject ใน component/page surfaces แล้ว.
