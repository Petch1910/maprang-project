...
- Structured knowledge Thai-first polish: pass via `knowledge:audit`, `vault:audit:test`, focused `knowledge.service.test.ts`, `eval:local`, `predeploy:check`, `secrets:check`, and `git diff --check`; knowledge audit now blocks old English structured snippets such as `Roleplay content is fictional simulation.` from returning.
