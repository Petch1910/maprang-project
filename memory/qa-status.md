...
- 2026-05-21 frontend chat stream malformed-event guard pass: `bun run frontend:api:test` ผ่าน 7 tests / 17 expects หลังเพิ่ม `parseChatStreamEvent`; `bun run predeploy:check:test`, `bun run predeploy:check`, `bun run frontend:check`, `bun run secrets:check`, และ `git diff --check` ผ่าน.
