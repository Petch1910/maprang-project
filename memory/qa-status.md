...
- API route audit now scans `apps/backend/index.ts`, covers `GET /`, and API smoke plus browser e2e smoke verify the root `maprang-backend` identity response; `api:audit:test` now locks that root coverage.
