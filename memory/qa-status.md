...
- 2026-05-21 full repo QA pass after frontend auth classifier hardening: `bun run qa:repo` ผ่านครบหลังเพิ่ม `authClassifierText`, static audit rule กัน raw auth error lower-case classifier, และ predeploy guard โดยรวม backend 166 tests / 566 expects, frontend static audit 17 tests, predeploy check 3 tests / 57 expects, import-cycle audit 127 source files / 305 import edges, API route audit 48 routes, route/menu audit 14 surfaces, frontend bundle budget main 269.1KB/350KB กับ chat 228.5KB/260KB.
- 2026-05-21 frontend auth classifier hardening pass: `bun test scripts\frontend-static-audit.test.ts scripts\predeploy-check.test.ts` ผ่าน 20 tests / 82 expects, `bun run frontend:check` ผ่าน static/route audit, eslint, TypeScript, Vite build และ bundle budget, และ `bun run predeploy:check`/`bun run secrets:check`/`git diff --check` ผ่านหลังบล็อก `error.message.toLowerCase()` ใน AuthPanel.
