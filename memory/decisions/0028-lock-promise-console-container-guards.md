...
รูปแบบที่ต้องระวังคือโค้ดที่ดูเหมือน registry หรือ helper map เช่น `[safeReject, Promise.reject]`, `([console.error])`, `new Map([['reject', Promise.reject]])`, `new Map([['console', console]])`, `{ Promise }`, `{ fallback, Promise }`, `{ console }`, และ `{ safeLogger, console }` เพราะ syntax เหล่านี้ทำให้ raw object path ถูกเก็บไว้แล้วส่งต่อภายหลังได้ยากต่อการ review ด้วยสายตา
...
- Coverage ต้องรวม object property, array entry, later array entry, parenthesized array, `new Map` tuple entry, object-literal shorthand, และ parenthesized object-literal shorthand
