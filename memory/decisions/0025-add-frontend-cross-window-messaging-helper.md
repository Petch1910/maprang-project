...
- frontend static audit ต้องบล็อก direct `postMessage(...)` นอก helper ทุกแบบ ไม่ใช่เฉพาะ wildcard และต้องบล็อก direct `message` listener นอก helper

## ผลลัพธ์

- Future UI มีทางใช้ cross-window messaging ที่ปลอดภัยและทดสอบได้
- Component/page ไม่ต้องถือ logic ตรวจ origin เอง
- Static QA ป้องกัน regression ทั้ง direct postMessage และ direct message listener ที่ไม่ผ่าน helper
