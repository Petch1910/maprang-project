...
- `CORS_ORIGINS` ต้องเป็น frontend HTTPS origin จริงเท่านั้น และต้องไม่ใช้ localhost, `127.0.0.1`, `0.0.0.0`, `::1`, `http://`, wildcard, path/query/hash, หรือ backend URL
...
- CORS เป็น frontend HTTPS origin จริงเท่านั้น ไม่ใช่ localhost/loopback, `http://`, wildcard, path/query/hash, หรือ backend URL
