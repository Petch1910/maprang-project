...
`OPENROUTER_API_KEY` powers creator text drafting and chat responses. Real creator avatar generation is a separate image-provider path; if `IMAGE_GENERATION_API_KEY` is not configured, Creator Studio will still draft Thai character content but will label the avatar as a temporary system placeholder.

`MODEL_TEMPERATURE` and `MODEL_MAX_OUTPUT_TOKENS` control chat feel. Keep `MODEL_MAX_OUTPUT_TOKENS` around `900` for roleplay so bots have room to answer with 2-4 short paragraphs instead of one-line replies.
...
`production:check` is the hard final gate. It fails if the backend URL is still local, auth is not Supabase JWT, avatar storage is not Supabase signed URL, CORS still points at localhost, OpenRouter is missing, or the image generation provider is missing.
...
`smoke:image` checks the image provider configuration without spending image credits by default. To generate one real staging/production avatar, run `SMOKE_IMAGE_LIVE=1 bun run smoke:image`; this calls `/creator/ai-draft` and fails if Creator Studio falls back to the placeholder image.
...
For a single production smoke gate, run `bun run smoke:live` with the same `SMOKE_API_BASE_URL` and smoke auth variables set. It verifies image provider configuration and live chat. Add `SMOKE_IMAGE_LIVE=1` only when you also want that gate to generate a real image.
...
- Live image smoke is opt-in through `SMOKE_IMAGE_LIVE=1` so routine smoke checks do not spend image credits by accident.
