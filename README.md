...
`api:smoke:live` จะพิมพ์ field หลักฐานแชทแบบคัดลง handoff ได้ตรง ๆ ในแถว `POST /chat/stream live` และรวมหลักฐาน live provider ที่ผ่านจริงไว้ใน JSON summary `handoffEvidence`: `Chat smoke normal chatId`, `Chat smoke normal tokens`, `Chat smoke normal walletTransactionId`, `Chat smoke stream chatId`, `Chat smoke stream tokens`, และ `Chat smoke stream walletTransactionId`.
