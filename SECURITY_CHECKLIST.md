...
- Smoke/diagnostic scripts ที่แสดง response payload, fetch failure หรือ caught error รวมถึง `predeploy:check`, `deploy:doctor`, `deploy:status`, `smoke:doctor`, `e2e:smoke`, `api:smoke`, `smoke:ready`, `smoke:image`, `smoke:chat`, `smoke:local`, `supabase:storage:check/setup`, structured knowledge health diagnostics, และ database health/DB-check diagnostics ต้องใช้ `formatDiagnosticText`/`formatPayload` หรือ `redactSensitiveText` เพื่อ redact secret-shaped value ก่อนเขียน log หรือคืน error.
