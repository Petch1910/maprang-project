...
- Secret hygiene / สุขอนามัยของ secret: `secrets:check` สแกน source/docs ที่ commit แล้วเพื่อหา OpenRouter, OpenAI, Anthropic, Hugging Face, Stripe live keys, platform tokens, JWT-like secrets, และ fail ถ้ามีไฟล์ `.env`/`.env.*` ที่ถูก track; env local ที่ไม่ถูก track ยังใช้พัฒนาได้ตามปกติ. ใช้ `bun run secrets:history` ก่อนเปิด repo หรือส่งมอบให้คนนอกเพื่อสแกน Git history แบบไม่พิมพ์ค่า secret; ถ้าพบรายการใน history ต้อง rotate/revoke key และพิจารณา rewrite history ก่อนเผยแพร่.
- Smoke/diagnostic scripts ที่แสดง response payload, fetch failure หรือ caught error รวมถึง `predeploy:check`, `deploy:doctor`, `deploy:status`, `smoke:doctor`, `e2e:smoke`, `api:smoke`, `smoke:ready`, `smoke:image`, `smoke:chat`, `smoke:local`, `supabase:storage:check/setup`, structured knowledge health diagnostics, database health/DB-check diagnostics, chat provider classification, และ Creator Draft retry classification ต้องใช้ `formatDiagnosticText`/`formatPayload`/`formatUnknownDiagnosticText` หรือ `redactSensitiveText`/`redactUnknownDiagnosticText` เพื่อ redact secret-shaped value และหลีกเลี่ยงการ stringify object-shaped error ดิบก่อนเขียน log คืน error หรือจัดประเภท provider failure.
