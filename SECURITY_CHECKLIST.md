...
- Secret hygiene: `secrets:check` scans committed source/docs for provider keys and fails if any tracked `.env`/`.env.*` file is present; untracked local env files stay usable for development.
