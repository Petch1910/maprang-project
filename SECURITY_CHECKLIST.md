...
- Frontend auth/provider error classifier ต้องไม่ใช้ `error.message.toLowerCase()` หรือ `String(error).toLowerCase()` ตรงๆ; ให้ผ่าน helper ที่ sanitize หรือแปลงเป็นข้อความที่ควบคุมได้ก่อนเสมอ
